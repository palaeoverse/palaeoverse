test_that("time_bins() works with macrostrat", {
  ...
  expect_message(
    expect_equal(
      nrow(time_bins(size = 10, scale = "North american land mammal ages")),
      7
    )
  )
})
...
test_that("time_bins() works", {
  ...
  expect_message(
    expect_equal(
      nrow(time_bins(interval = c(0, 200), rank = "period", size = 20)),
      5
    )
  )
  expect_message(
  ...
  expect_equal(
    nrow(time_bins(interval = c("Albian", "Danian"), rank = "stage")),
    8
  )
  expect_equal(
    nrow(time_bins(
      interval = c("Permian", "Danian"),
      rank = "period",
      scale = "GTS2012"
    )),
    5
  ...
  expect_message(
    expect_equal(
      nrow(time_bins(interval = c("Phanerozoic"), size = 1000, rank = "stage")),
      1
    )
  )
