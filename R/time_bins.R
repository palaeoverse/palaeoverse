...
#' In addition, near equal-length time bins can be generated by grouping stages together. In this implementation, intervals are grouped together in a way that minimises the mean bin
#' length between bins based on the user's specified bin size (i.e. if a user specifies 10 Myr as their target bin size, the function will generate groups of bins that have a mean bin
