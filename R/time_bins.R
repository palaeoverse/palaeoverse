...
#' used, or a `data.frame` of a geological time scale can be provided. In
...
#'   - \code{\link{GTS2012}}
#'
#'   Available intervals names are accessible via the `interval_name` column
...
time_bins <- function(interval = "Phanerozoic", rank = "stage", size = NULL,
  ...
  if (!scale %in% c("GTS2020", "GTS2012", "user")) {
    ...
    df$bin <- seq_len(nrow(df))
    # Reorder dataframe
    df <- df[, c("bin", "interval_name", "max_ma", "mid_ma",
                 "min_ma", "duration_myr", "abbr", "colour")]
  ...
  # Clean up --------------------------------------------------------------
  if (!scale %in% c("GTS2020", "GTS2012") && !is.null(size)) {
