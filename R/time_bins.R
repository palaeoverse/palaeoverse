#' Generate time bins
#'
#' A function to generate time bins for a given study interval. This function is flexible in that either stage-level or higher stratigraphic-level (period) time bins can be generated.
#' In addition, near equal-length time bins can be generated by grouping stages together. In this implementation, intervals are grouped together in a way that attempts to create
#' near equal-length time bins based on the user's specified bin size (i.e. if a user specifies 10 Myr as their target bin size, the function will generate groups of bins that have a mean bin
#' length as close to 10 Myr as possible). However, users may also wish to group stages based on subjective reasoning e.g.availability of outcrop.
#'
