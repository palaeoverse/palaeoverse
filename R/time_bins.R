#' Generate time bins
#'
#' A function to generate time bins for a given study interval. This function is flexible in that either stage-level or higher stratigraphic-level (period) time bins can be called.
#' In addition, near equal-length time bins can be generated by grouping stages together. In this implementation, intervals are grouped together in a way that creates near
#' equal-length time bins based on the user's specified bin size (e.g., if a user specifies 10 Myr as their target bin size, the function will generate groups of bins that have a mean bin
#' length close to 10 Myr). However, users may also want to consider grouping stages based on other reasoning e.g. availability of outcrop (see Dean et al. (2020); \doi{https://doi.org/10.1111/pala.12492}).
#'
...
#' - GTS2020: \url{https://stratigraphy.org/ICSchart/ChronostratChart2020-03.pdf}
#' - GTS2012: \url{https://stratigraphy.org/ICSchart/ChronostratChart2012.pdf}
#'
#' Available intervals names are accessible via GTS2020$interval_name or GTS2012$interval_name. Data from the Geological Timescale 2020 and 2012 were compiled by Lewis A. Jones (2022-07-02).
#' @section Developer(s):
#' Lewis A. Jones
#' @section Reviewer(s):
#' Kilian Eichenseer
...
time_bins <- function(interval = c("Fortunian", "Meghalayan"), size = NULL, assign = NULL, scale = "GTS2020", plot = FALSE){
  ...
  if(length(rnk) != 1){stop("Interval ranks should be the same, e.g. specifiy only stage or period names, not a combination.")}
  ...
  if(is.numeric(size) == TRUE){
    ...
    #generate dataframe
    grouping_rank <- rnk
    df <- cbind.data.frame(bin, max_ma, mid_ma, min_ma, duration_myr, grouping_rank, intervals)
  ...
  if(plot == TRUE){
    if(is.numeric(size) == TRUE){df$colour <- c("#4292c6")}
    ...
    if(is.numeric(size) == FALSE){
      df$bin <- nrow(df):1
      df <- df[,c("bin","interval_name", "max_ma", "mid_ma", "min_ma", "duration_myr", "rank")]
    }
