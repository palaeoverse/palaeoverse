...
#' [Paleobiology Database] (https://paleobiodb.org/#/) occurrence dataframe,
#' species names are only captured in the 'accepted_name' column, so a
#' species-level filter should use '`genus` = "genus"' and '`name` =
#' "accepted_name"' arguments. If a genus-level filter is desired, the minimum
#' input requires either (1) `binomial` or (2) `genus` columns to be entered, as
#' well as a `family` column.
#'
